...
const StackedBar: React.FC<StackedBarProps> = (props) => {
    ...
    const [stackWidths, setStackWidths] = useState<number[]>([0.1, 0.1, 0.1, 0.3])
    ...
    useEffect(() => {
        ...
        const sum = props.indicator.keyFigures.map(fig => fig.points).reduce((a, b) => a + b, 0);
        
        // calculate the stack Widths
        const stackWidths = props.indicator.keyFigures.map(fig => fig.points / sum);
    ...
    return (
        <Box style={{
            ...
            borderRadius: props.rounded ? `${(props.height ? props.height : 2) * 0.3}rem` : 'none',
